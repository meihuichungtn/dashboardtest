/* ==========================================================================
   ...
   1. 隱藏「Google 私密 API」提示文字，保持乾淨經費單位標籤
   2. 年度篩選器預設值動態對應資料庫內的最近一年 (Latest Year)
   ========================================================================== */
...
// --- Initialization with Auto-Detect + 0-Second Cache ---
document.addEventListener('DOMContentLoaded', () => {
  initData();
});

function initData() {
  const savedApiUrl = (typeof CONFIG !== 'undefined' ? CONFIG.googleAppsScriptUrl : '') || localStorage.getItem('googleAppsScriptUrl');
  const cachedRecordsStr = localStorage.getItem('cached_dashboard_records');
  const cachedTimestamp = localStorage.getItem('cached_dashboard_last_updated');

  // Step 1: 0-second instant render if local browser cache exists
  if (cachedRecordsStr) {
    try {
      const cachedData = JSON.parse(cachedRecordsStr);
      if (Array.isArray(cachedData) && cachedData.length > 0) {
        rawRecords = cachedData;
        onDataLoaded(); // 0ms instant render!
      }
    } catch (e) {
  ...
  // Step 2: If Google Apps Script API URL is configured, auto-detect version or sync in background
  if (savedApiUrl && savedApiUrl.trim() !== '') {
    syncWithGoogleAppsScript(savedApiUrl, cachedTimestamp);
  } else if (rawRecords.length === 0) {
    // Step 3: Fallback if no cache and no API URL -> load from data.js or data.json
...
// --- High Speed Auto-Detect Version Check & Background Sync ---
function syncWithGoogleAppsScript(apiUrl, currentCachedTimestamp, isForced = false) {
  // Add unique timestamp (_t) & cache: 'no-store' to bypass browser HTTP caching
  const cacheBuster = `_t=${Date.now()}`;
  const checkUrl = apiUrl.includes('?') ? `${apiUrl}&action=checkVersion&${cacheBuster}` : `${apiUrl}?action=checkVersion&${cacheBuster}`;

  fetch(checkUrl, { cache: 'no-store' })
    .then(res => res.json())
    .then(verJson => {
      const remoteLastUpdated = verJson ? String(verJson.lastUpdated || '') : '';
      
      // If remote timestamp is newer, cache is empty, or user clicked force sync!
      if (isForced || !currentCachedTimestamp || (remoteLastUpdated && remoteLastUpdated !== currentCachedTimestamp)) {
        fetchFullDatasetFromApi(apiUrl, remoteLastUpdated);
      }
    })
    .catch(err => {
      if (isForced || rawRecords.length === 0) {
        fetchFullDatasetFromApi(apiUrl, '');
      }
    });
}

function fetchFullDatasetFromApi(apiUrl, remoteLastUpdated) {
  const cacheBuster = `_t=${Date.now()}`;
  const fullUrl = apiUrl.includes('?') ? `${apiUrl}&${cacheBuster}` : `${apiUrl}?${cacheBuster}`;
  ...
  fetch(fullUrl, { cache: 'no-store' })
    ...
    .then(json => {
      if (json && json.data && Array.isArray(json.data) && json.data.length > 0) {
        ...
        const newTimestamp = String(json.lastUpdated || remoteLastUpdated || Date.now());

        try {
          localStorage.setItem('cached_dashboard_records', JSON.stringify(json.data));
          localStorage.setItem('cached_dashboard_last_updated', newTimestamp);
        } catch (e) {
          console.warn("StorageQuotaExceeded when caching:", e);
        }
        ...
        if (badge) {
          ...
          setTimeout(() => badge.style.background = 'rgba(16, 185, 129, 0.12)', 1500);
        }
      } else if (rawRecords.length === 0) {
    ...
    .catch(err => {
      ...
      if (rawRecords.length === 0 && typeof DASHBOARD_DATA !== 'undefined') {
        rawRecords = DASHBOARD_DATA;
        onDataLoaded();
      }
    });
...
function forceSyncFromGoogle() {
  const savedApiUrl = (typeof CONFIG !== 'undefined' ? CONFIG.googleAppsScriptUrl : '') || localStorage.getItem('googleAppsScriptUrl');
  if (!savedApiUrl || savedApiUrl.trim() === '') {
    alert("未設定 Google Apps Script Web App URL！請在 config.js 或程式碼中設定網址。");
    return;
  }

  const badge = document.getElementById('dataStatusBadge');
  if (badge) {
    badge.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> 正在從雲端同步最新數據...`;
  }

  // Clear current cached timestamp and fetch directly with cache-busting
  localStorage.removeItem('cached_dashboard_last_updated');
  syncWithGoogleAppsScript(savedApiUrl, null, true);

  setTimeout(() => {
    if (badge) {
      badge.innerHTML = `<i class="fa-solid fa-circle-check"></i> 經費單位：新台幣元 (已同步)`;
    }
  }, 1800);
}

function onDataLoaded() {
  // Extract all distinct years and sort DESCENDING (e.g. 114, 113, 112...)
  ...
  // Dynamically default filter to the LATEST year present in the data (allYears[0])
  const latestYear = allYears.length > 0 ? allYears[0] : 114;
  
  ['ranking', 'pivot', 'detail'].forEach(tabId => {
    filterState[tabId].years = new Set([latestYear]);
...
// ===================================================================
// TAB 4: SINGLE SCHOOL TREND (單校歷年變動趨勢)
// ===================================================================
